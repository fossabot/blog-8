...
- administrator - ma on pełnię uprawnień, w celach bezpieczeństwa jest on wylogowywany automatycznie po okresie bezczyności
...
Do uruchomenia aplikacji potrzebne jest zaistalowany Python w wersji co najmniej 3.6. Można go zaistalować poniższym poleceniem:
...
Następnie wymagane jest zaistalowanie zależności aplikacji. Można to wykonać poniszymi poleceniami:
...
pipenv shell
```
Kolejnym krokiem jest przełączenie na skonfigurowanie środowisko wirtualne, stworzeniem bazy danych i wygeneranie plików statycznych:
...
Strona powinna uruchomić się uruchomić i działać pod portem 8080. Jeśli chesz możesz uruchomić aplikację pod innym portem. Aby zweryfikować  jej działanie wejdź na adres http://127.0.0.1:8080/  za pomocą przeglądarki.
...
Aby zalogować się na konto administratora wejdź na adress `http://localhost:8000/admin/` i zaloguj się za pomocą wcześniej ustanowionego hasła. Przed twoimi oczami ukarze się panel administracyjny.
...
Po zalogowania masz możliwość dodawania postów. Aby tego dokonać wróć na stronę główną. I kliknij na `Dodaj Post` w pasku nawigacyjnym. 
W nowym okienku wyświetli się edytor. Dodaj tytuł swojego pierszego posta i wybierz datę kiedy ma się on ukazać. Możesz np. ustawić, że post ma się pojawić jutro, lub mieć wczorajszą datę. Za pomocą następego pola masz możliwość edycji treści posta. Możesz za pomocą edytora np. dodać do posta zdjęcie lub dołączyć do niego film na Youtubie.
...
Zostaniesz przekierowany na nowoutworząną stronę. Jeśli jesteś zalogowany masz możlowość edycji postu, klikając na  ikonę notesu. Zaawansowana edycja oraz oraz usunięcie strony można wykonać za pomocą panelu administracyjnego.


# 8. Facebook i Komentarze
Aby umożliwić dodawanie komentarzy, jak i zintegrować aplikację z Facebookiem, zerejestruj ją na strone `https://developers.facebook.com/` Następnie dodaj uzyskany numer aplikacji do panelu administracyjnego klikając dodaj przy `Facebook identyfikator aplikacji`. Po zapisaniu zmian aplikacja powinna być już zintegrowana z serwisem.
...
python manage.py createsuperuser 
```
Ponadto w panelu administrayjnym w sekcji `Użytkownicy`, masz możliwość edycji danych użytkownika.
...
## 12.2. Kontakt
Odblokuj stronę kontaktów na stronie ustawień, jak wejść w ustawienie jest zawarte w sekcji [6. Podstawowa Konfiguracja](#6-podstawowa-konfiguracja). Wejdź na stronę głowną aplikacji. Po prawej stronie będziesz miał informację o ilości nieprzeczytanych wiadomości ze strony kontaktowej. Po kliknięciu na przycisk informujęcy o wiadomościach zostaniesz przeniesiony do panelu administracyjnego. Masz dostęp do wszystkich wiadomości. Po kliknięciu na wiadomość masz możliwość jej przeczytania. Możesz również zaznaczyć, że ją przeczytałeś.
